...
**Key detail — Rate-Mode-only sheets (e.g. a `BASIC` care area):** a sheet whose every drug entry starts with `"Rate Mode"` is exempt from the drug-library checks (no concentration/limits/etc.), but **KVO Mode (col AT) and Quick Recall (col AV) are still required** on each Rate Mode entry. So the sheet-skip in the validate loop runs a focused Rule 5 pass for AT/AV before `continue`-ing past the other rules. (Previously the whole sheet was skipped, so missing AT/AV on a BASIC sheet went uncaught — reported on the Tift Regional file.)
