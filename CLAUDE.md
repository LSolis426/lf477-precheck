...
- **Inflated sheet ranges (fixed)** — Excel often reports a sheet's used range (`!ref`) as the full 1,048,576 rows when stray formatting exists on an otherwise-empty column, even though real data ends after ~50 rows. `validate()` previously iterated the whole reported range, taking ~70s and **freezing the browser tab** ("page isn't responding"). It now scans the worksheet's actual cells to find the last row that holds a *value* (cells with only formatting have no `.v`) and bounds `sheet_to_json` to that row — identical results, ~1600× faster. Real example: `JFK 3870 DERS Template 3 Care Areas.xlsx` (a sheet with `!ref=A1:AV1048576` but data only through row 56).
